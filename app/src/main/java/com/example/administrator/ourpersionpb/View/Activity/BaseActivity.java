...
import com.example.administrator.ourpersionpb.Model.bean.User;
import com.example.administrator.ourpersionpb.R;
import com.example.administrator.ourpersionpb.Utils.ToastUtils;
import com.example.administrator.ourpersionpb.View.Dialog.LoadDialog;
import com.example.administrator.ourpersionpb.zxing.android.CaptureActivity;
...
//  http://blog.csdn.net/lmj623565791/article/details/72859156
            imageUri = getUriForFile(BaseActivity.this, output);
    ...
    public void crop(Uri uri) {
        ...
        //--这个是图片剪切后的路径
        file = new File(Environment.getExternalStorageDirectory(), "剪切" + System.currentTimeMillis() + ".jpg");
        try {
            file.createNewFile();
        } catch (IOException e) {
            e.printStackTrace();
        }
        List<ResolveInfo> resInfoListtwo = getPackageManager()
                .queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY);
        for (ResolveInfo resolveInfo : resInfoListtwo) {
            String packageName = resolveInfo.activityInfo.packageName;
            grantUriPermission(packageName, getUriForFile(BaseActivity.this, file), Intent.FLAG_GRANT_READ_URI_PERMISSION
                    | Intent.FLAG_GRANT_WRITE_URI_PERMISSION);
        }
        intent.putExtra(MediaStore.EXTRA_OUTPUT, getUriForFile(BaseActivity.this, file));
    ...
    private Uri getUriForFile(Context context, File file) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            uri = FileProvider
                    .getUriForFile(context.getApplicationContext(), "com.example.administrator.ourpersionpb.fileprovider", file);
        } else {
